...
# Thay đổi nếu đường dẫn model khác
...
# Load model 1 lần khi import module
print("Đang load model, chờ chút...")
mo_hinh = load_model(MODEL_PATH)
print("Load model xong.")

# Lấy kích thước input từ model (nếu model có input shape)
try:
    INPUT_SHAPE = tuple(mo_hinh.input_shape[1:3])
except Exception:
    INPUT_SHAPE = (320, 320)  # fallback

# Danh sách lớp theo thứ tự khi train (phải khớp với model)
cac_lop = [
    'binh_thuong',
    'lua_bi_nam',
    'nho_bi_phan_trang',
...
def du_doan_benh(duong_dan_anh, nguong=NGUONG):
    ...
    # Nếu có dữ liệu tư vấn trong JSON
    info = thong_tin_benh.get(label, None)
    if info:
        # Tạo chuỗi text ngắn để hiển thị trên giao diện bot
        text_lines = []
